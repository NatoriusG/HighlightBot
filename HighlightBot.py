...
async def get_tags(link):
	...
	try:
		async with client.web_session.get(album_url, headers=IMG_HEAD) as web_response:
			...
			description = image_data['description'].split(' ')

			# Verify data and collect tags
			for substring in description:
				if substring.startswith('#'):
					tags.append(substring[1:])
